...
const translations = {
  zh: {
    ...
    "product.1.capability.1": "工程镜像同步",
    "product.1.capability.2": "规则与上下文整合",
    "product.1.capability.3": "AI 协作工作流",
  ...
  en: {
    ...
    "product.1.capability.1": "Engineering Mirror Sync",
    "product.1.capability.2": "Rules and Context",
    "product.1.capability.3": "AI Collaboration Workflow",
...
function renderArticleHub() {
  ...
  setContentIndexCurrent("#articles", isTopicView
...
function renderProductHub() {
  ...
  const productCard = productGrid.querySelector(".product-console-card");
  if (productCard && products[0]) {
    productCard.setAttribute("href", products[0].href || detailHash("products", products[0]));
    productCard.classList.toggle("active", Boolean(selected && selected.id === products[0].id));
  }
  ...
  detailPanel.innerHTML = `
    ...
    <div class="detail-panel-head">
      <small>${escapeHtml(translations[currentLang]["products.detail.label"])}</small>
      <h4>${escapeHtml(selected.title[currentLang])}</h4>
      <p>${escapeHtml(localizedText(selected.pain, currentLang))}</p>
    </div>
    <div class="detail-meta-grid">
      <div class="detail-meta-tile"><span>Version</span><strong>${escapeHtml(selected.version || "V1.0")}</strong></div>
      ${renderMetaTile("resources.detail.status", selected.status)}
      ${renderMetaTile("resources.detail.access", selected.access)}
    </div>
    <section class="detail-block">
      <strong>${escapeHtml(translations[currentLang]["products.detail.capability"])}</strong>
      ${renderInlineList(selected.capabilities[currentLang] || [])}
    </section>
    <section class="detail-block">
      <strong>${escapeHtml(translations[currentLang]["products.detail.workflow"])}</strong>
      ${renderInlineList(selected.workflow[currentLang] || [])}
    </section>
    <section class="detail-block detail-note">
      <strong>${escapeHtml(translations[currentLang]["products.detail.delivery"])}</strong>
      <p>${escapeHtml(localizedText(selected.delivery, currentLang))}</p>
    </section>
  `;
}
